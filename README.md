...
- [セットアップ:事前準備](#セットアップ事前準備)
  ...
  - [build-kikutan をテキトウなフォルダに展開](#build-kikutan-をテキトウなフォルダに展開)
- [使い方: サンプルとチュートリアルを通して説明します](#使い方-サンプルとチュートリアルを通して説明します)
  - [サンプルを使って実際作ってみよう。](#サンプルを使って実際作ってみよう)
...
<!-- <a src="imgs/j">このツールで生成したじぶんキクタン</a> -->
[link](img/sample-en_ja--words-1.mp3)
...
このツールを使うと、このテキストを音声化し、好きな音楽をバックグラウンドミュージックとしてミックスし、iPhone 等で聴くことが出来ます。  
ジョギングや、家事をしながらボキャビルが出来るようになるんです！やった！  
...
- `sox`: 音声結合で使用
- `eyeD`: 曲名、歌詞設定等で使用
- `rake`: `rake` コマンドは Ruby のタスク管理ツール。最近の macOS はデフォルトで使える。
...
## build-kikutan をテキトウなフォルダに展開

<img src="./imgs/tut/github-download-zip.png" width="400">

このページの右上の "Clone or download" から zip をダウンロードして展開する。

# 使い方: サンプルとチュートリアルを通して説明します

まず、このチュートリアルで、この音声を自力で自分で作れるようになる事を目標にする。
それが出来たら、後は自分で用意した単語リストや例文リストを使って音声合成出来るはずだ。

## サンプルを使って実際作ってみよう。

ターミナルを開いて build-kikutan を展開したフォルダに移動しよう。  
作業は殆ど、ターミナルから行う。  
ここから、サンプルプロジェクトを使って実際にキクタン的なアルバムを作成していく。
入門から応用まで説明するので、個々を乗り越えれば余裕でオリジナル・キクタンを作れるようになる！
...
├── sample-en_ja.png       # アルバムジャケット en_ja 用
├── sample-ja_en.png       # アルバムジャケット ja_en 用
├── words-1.txt            # 単語リスト-1
└── words-2.txt            # 単語リスト-2
```

作業は `rake` というコマンドを使って進めていく。  
`conf` と、`src` は常に指定する必要がある。  
作業は `Rakefile` があるディレクトリで行うこと。  

早速やってみよう。サンプルの `config.yml` と 単語リスト-1(`words-1.txt`)を指定したら `-T` オプションでタスクの一覧が見られる。
...
基本ルールとして、`en_ja:album` は `en_ja:album` を作成する。`en_ja:album:play` とすると作成したファイルを即座に再生する。  
細かく成果物を確認したい時は `:play` を末尾につける、と覚えておけば良い。  
...
できた。何が起こった？
設定ファイル(`conf`)と指定された単語リスト(`src`) からGoogleで音声化し、結合し、バックグラウンドミュージックを合成し、アルバムジャケットを設定した。
...
設定ファイルはコレ。`#` でコメントアウトされた部分は無視して良いので、消してある。
...
`src/sample` 配下に新しく出来たディレクトリとファイルはこれだ。

ここでフォルダの役割と生成の順序を説明しておく。ここの理解は重要だ。
```
.
├── out
...
1. `raw`: Google TTS で取得した生の音声。text と voice 定義(en-US-Wavenet-F 等)の hash 値をファイル名にしている。同じプロジェクトであれば、"テキスト、voice"のペアが同じなら音声は再利用される。このフォルダを消してしまうと、また再生成が必要なので、デフォルトでフォルダの場所を変えている。out 配下をまるっと消しても raw は残るように。
...
5. `out/mp3`: サイズ縮小のため、mp3 化したもの。このタイミングで指定があればバックグラウンドミュージックとミックスする。
...
`clean` は `raw` ディレクトリ以外のディレクトリ `concat`, `compile`, `mp3` を削除する。
...
SVLの 1000語とかだと１時間以上かかるので、長い曲を作る必要がある。`soxi`で曲の長さを調べ、必要なループ回数を計算して、`repet X` でリピートする。
